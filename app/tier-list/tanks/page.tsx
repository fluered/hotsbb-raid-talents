...
import { getWclToken, getRaidStructure, MIDNIGHT_RAIDS, TANK_SPECS, getSeasonState } from '../../../lib/wow';
...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  const title = bossName
    ...
    : `${season.expansionName} ${season.seasonLabel} Tank DPS Tier List — All Bosses | HotsBB`;
  const description = bossName
    ? `Tank spec tier list for ${diffLabel} ${bossName} — ranked by avg DPS of top 50 parses.`
    : `Overall tank spec tier list for WoW ${season.expansionName} ${season.seasonLabel} — avg DPS across all raid bosses, ranked from top parses.`;
  return {
    title,
    description,
    ...ogImageMeta({
      ...
      <header className="border-b border-zinc-800/70 bg-black/70 backdrop-blur-md sticky top-0 z-50">
        <div className="px-4 md:px-5 h-12 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <Link href={`/tier-list/tanks?difficulty=${activeDifficulty}${activeRegion !== 'global' ? `&region=${activeRegion}` : ''}`} className="hidden sm:inline text-xs font-black text-zinc-300 hover:text-zinc-100 uppercase tracking-widest transition-colors">Tank Tier List</Link>
          </div>
        ...
        {(() => {
          ...
          return (
            <div className="flex items-center gap-1 bg-zinc-900/60 rounded-xl p-1 border border-zinc-800/60 self-start w-fit">
              <Link href={`/tier-list?${qs}`} className="px-5 py-1.5 rounded-lg text-sm font-bold text-zinc-500 hover:text-zinc-300 transition-colors">DPS</Link>
              <span className="px-5 py-1.5 rounded-lg text-sm font-bold bg-zinc-700/60 text-zinc-100">Tanks</span>
        ...
        {selectedBoss ? (
          ...
          <Suspense fallback={<TierListSkeleton />}>
            <OverallTierListContent
              ...
              region={activeRegion}
              role="tank"
              title={`${season.expansionName} ${season.seasonLabel} Raid Tank DPS Tier List`}
