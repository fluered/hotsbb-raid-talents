      ...
      query {
        reportData {
          report(code: "${reportCode}") {
            ...
            events(fightIDs: [${fightId}], dataType: CombatantInfo, startTime: 0, endTime: 2147483647) {
              data
            }
          }
        }
...
async function fetchCompleteSpecLayoutMatrix(wclToken: string, className: string, specName: string) {
  try {
    const query = `query { gameData { class(name: "${className.replace(/\s+/g, '')}") { specialization(name: "${specName.replace(/\s+/g, '')}") { talents { id name maxRank row column } } } } }`;
    const response = await fetch('https://www.warcraftlogs.com/api/v2/client', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${wclToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ query }),
    ...
    const result = await response.json();
    return result.data?.gameData?.class?.specialization?.talents || [];
  } catch (e) { return []; }
}
...
export default async function Home(props: PageProps) {
  ...
  const activeDifficulty = searchParams.difficulty ? parseInt(searchParams.difficulty) : 4;
  const activeRegion = searchParams.region || 'US';

  let zones = [];
  let detailedRankings = [];
  let skeletonMap = [];
  let error = null;
      ...
      <header className="border-b border-zinc-800/80 bg-zinc-900/40 backdrop-blur-md px-8 py-4 flex flex-col md:flex-row gap-4 justify-between items-center shrink-0 shadow-sm sticky top-0 z-50">
        <div className="flex items-center space-x-3.5">
          ...
          <h1 className="text-xl font-black tracking-tight text-zinc-50">
            MURLOK<span className="text-amber-500 font-medium">RAID</span>
          </h1>
        </div>
            ...
            {POPULAR_SPECS.map((cls) => {
              ...
              return (
                <Link
                  ...
                  className={`block w-full text-left text-sm py-2 px-3 rounded-xl transition-all border ${
                    isClassSelected ? `${cls.activeBg} ${cls.border} ${cls.color} font-black shadow-sm` : 'text-zinc-400 border-transparent hover:text-zinc-200 hover:bg-zinc-900/40'
                  }`}
                >
                  {cls.class}
                </Link>
        ...
        <main className="md:col-span-3 flex flex-col space-y-6 min-h-0 w-full">
          ...
          {!activeClass ? (
            <div className="bg-zinc-900/10 border border-dashed border-zinc-800 p-12 rounded-2xl text-center text-zinc-500 flex flex-col items-center justify-center space-y-3">
              ...
              <h3 className="text-zinc-200 font-bold text-md">Raid Blueprint Terminal Idle</h3>
              <p className="text-xs text-zinc-500 max-w-sm leading-relaxed">
                Select an entry from the Class console on the left to initialize specializations and live fight analysis charts.
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="space-y-3 shrink-0">
                <h2 className="text-xs font-bold uppercase tracking-widest text-zinc-500 px-1">1. Select Specialization</h2>
              ...
              {activeSpec && (
                <div className="space-y-3 shrink-0">
                  <h2 className="text-xs font-bold uppercase tracking-widest text-zinc-500 px-1">2. Encounter Target Selection</h2>
              ...
              <div className="space-y-3 w-full">
                <div className="flex justify-between items-center px-1">
                  <h2 className="text-xs font-bold uppercase tracking-widest text-zinc-500">
                    3. Historic Roster Blueprint Matrix
                  </h2>
                </div>
                        ...
                        return (
                          <div key={idx} className="bg-zinc-950/50 border border-zinc-800/60 p-5 rounded-xl flex flex-col gap-4 shadow-sm group">
                            <div className="flex justify-between items-center border-b border-zinc-900 pb-3">
                              ...
                              <a href={`https://www.warcraftlogs.com/reports/${player.report?.code}#fight=${player.report?.fightID}&source=${player.telemetry?.sourceId}`} target="_blank" className="text-[10px] bg-zinc-800 px-3 py-1 rounded">WCL ↗</a>
                            </div>
                            <div className="text-[11px] font-mono text-zinc-400">
                                <span className="text-emerald-400 font-bold">{Math.round(player.amount).toLocaleString()}</span> Output
                            </div>
                          </div>
                        );
                      })
